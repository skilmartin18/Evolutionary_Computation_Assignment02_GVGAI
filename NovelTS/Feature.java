        ...
        for (int x = 0; x < width; x++) {
            for (int y  = 0; y < height; y++) {
                ...
                for (Observation obs: observations) {
                    ...
                    Atom atom;
                    int distance = Utils.compareDistance(pos, playerFeature)
                    / IWPlayer.blocksize;

                    if (Param.featureSelection == Param.Feature.ITYPE) {
                        atom = new Atom((int)pos.x, (int)pos.y, obs.itype);
                    }
                    else if (Param.featureSelection == Param.Feature.CATEGORY) {
                        atom = new Atom((int)pos.x, (int)pos.y, obs.category);
                    }
                    else if (Param.featureSelection == Param.Feature.DISTANCE) {
                        atom = new Atom((int)pos.x, (int)pos.y, distance);
                    } else {
                        atom = null;
                    }

                    // Add object with close distance to closeObject array
                    if (distance <= Param.closeDistance) {
