...
public class IWPlayer {
    ...
    public IWPlayer(Random random) {
        ...
        if (Param.randomAction) {
            randomAction = new RandomAction(Agent.NUM_ACTIONS, 100, random);
        } else {
            randomAction = new RandomAction(Agent.NUM_ACTIONS, 1, random);
        }
        numRun = 0;
        totalDepth = 0;
    ...
    public void init(StateObservation gameState) {
        if (serializer == null) {
            serializer = new Serializer(gameState);
        }
        ...
        rootNode.updateScore(value(gameState), heuristic(feature));

        if (Param.noveltyPreference) {
            noveltyTable.getNovelty(feature, 0);
        } else {
    ...
    */
    ...
    private TreeNode expand(TreeNode parentNode, StateObservation currentObservation) {
        ...
        double novelty = 0;
        if (Param.noveltyPreference) {
            novelty = noveltyTable.getNovelty(feature, (int) score);
        } else {
            novelty = noveltyTable.getNovelty(feature);
        }

        // Consider all rewarding action as novel
        if (Param.rewardNovelty) {
            if (score > 0) {
                novelty = 1;
            }
        }
        ...
        newNode.updateScore(score, heuristic(feature));
    ...
    private void backPropagate(TreeNode node) {
        ...
        if (Param.goalSerializer) {
            if (node.checked == false && node.numchild != 0) {
                serializer.updateHeuristic(node.feature, node.reward);
                // node.checked = true;
            }
        }
        node.update();
     ...
     */
    private double heuristic(Feature feature) {
        double h = 0;
        if (Param.distanceHeuristic) {
            h += positionMatrix.getScore(feature);
        }

        if (Param.goalSerializer) {
            h += serializer.getScore(feature) * Param.exploreExploitRate;
        }
        return h;
